      ...
      if(ext === 'js'){
        ...
        var loadFunction = function(next){
          self.api.watchFileAndAct(file, function(){
            self.api.log('server', 'rebooting due to initializer change (' + file + ')', 'info');
            self.api.commands.restart.call(self.api._self);
          });

          if(typeof self.initializers[initializer].initialize === 'function'){
            if(typeof self.api.log === 'function'){ self.api.log('server', 'loading initializer: ' + initializer, 'trace', file); }
            self.initializers[initializer].initialize(self.api, function(err){
              try{ self.api.log('server', 'loaded initializer: ' + initializer, 'trace', file); }catch(e){ }
              next(err);
            });
          }else{
            next();
        ...
        var startFunction = function(next){
          if(typeof self.initializers[initializer].start === 'function'){
            if(typeof self.api.log === 'function'){ self.api.log('server', 'start: ' + initializer, 'debug', file); }
            self.initializers[initializer].start(self.api, function(err){
              self.api.log('server', 'started: ' + initializer, 'debug', file);
              next(err);
            });
          }else{
            next();
        ...
        var stopFunction = function(next){
          if(typeof self.initializers[initializer].stop === 'function'){
            if(typeof self.api.log === 'function'){ self.api.log('server', 'stop: ' + initializer, 'debug', file); }
            self.initializers[initializer].stop(self.api, function(err){
              self.api.log('server', 'stoped: ' + initializer, 'debug', file);
              next(err);
            });
          }else{
            next();
...
actionhero.prototype.stop = function(callback){
  ...
  if(self.api.running === true){
    ...
    self.api.log('server', 'shutting down open servers and stopping task processing', 'alert');
    ...
    self.stopInitializers.push(function finalStopInitializer(){
      ...
      self.api.log('server', 'actionhero has been stopped', 'alert');
    ...
    self.api.log('server', 'cannot shut down actionhero, not running', 'error');
    if(typeof callback === 'function'){ callback(null, self.api) }
  }
};
...
actionhero.prototype.restart = function(callback){
  ...
  if(self.api.running === true){
    self.stop(function(err){
      if(err){ self.api.log('server', err, 'error'); }
      self.start(self.startingParams, function(err){
        if(err){ self.api.log('server', err, 'error'); }
        self.api.log('server', 'actionhero restarted', 'info');
        if(typeof callback === 'function'){ callback(null, self.api) }
      });
    });
  } else {
...
var fatalError = function(api, errors, type){
  ...
  if(errors){ 
    api.log('server', 'error with initializer step: ' + type, 'emerg');
    errors.forEach(function(err){
      api.log('server', err.stack, 'emerg');
    });
    process.exit(1); 
  }
}
