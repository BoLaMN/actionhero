    ...
    api.resque = {
      ...
      startQueue: function(callback){
        ...
        self.queue.on('error', function(error){
          api.log('resque', error, 'error', '[api.resque.queue]')
        });
      ...
      startScheduler: function(callback){
        ...
        if(api.config.tasks.scheduler === true){
          ...
          self.scheduler.on('error', function(error){
            api.log('resque', error, 'error', '[api.resque.scheduler]')
          });
      ...
      startMultiWorker: function(callback){
        ...
        self.multiWorker.on('internalError',     function(error){                         api.log('resque', error, 'error'); })
        self.multiWorker.on('multiWorkerAction', function(verb, delay){                   api.log('resque', 'checked for worker status: ' + verb + ' (event loop delay: ' + delay + 'ms)', 'trace'); })
      ...
      stopMultiWorker: function(callback){
        ...
        if(api.config.tasks.minTaskProcessors > 0){
          self.multiWorker.stop(function(){
            api.log('resque', 'task workers stopped');
            callback();
          });
        }else{
          callback();
